...
ctcStatus_t GpuCTC<ProbT>::launch_alpha_beta_kernels(const ProbT* const log_probs,
    ...
    if (compute_alpha)
        hipLaunchKernelGGL((compute_alpha_kernel<ProbT, NT, VT>), dim3(grid_size), dim3(NT), 0, stream_, log_probs, label_sizes_, utt_length_,
    ...
    if (compute_beta) {
        hipLaunchKernelGGL((compute_betas_and_grad_kernel<ProbT, NT, VT>), dim3(grid_size), dim3(NT), 0, stream_, log_probs, label_sizes_, utt_length_, repeats_,
...
GpuCTC<ProbT>::compute_log_probs(const ProbT* const activations) {
    ...
    hipLaunchKernelGGL((prepare_stable_LSM_kernel<ProbT, VT>), dim3(grid_size), dim3(NT), 0, stream_, ctc_helper::identity<ProbT>(), log_probs_,
    ...
    hipLaunchKernelGGL((compute_log_probs_kernel<ProbT, VT>), dim3(grid_size), dim3(NT), 0, stream_, ctc_helper::identity<ProbT>(), log_probs_,
         denoms_, out_dim_, num_elements);
